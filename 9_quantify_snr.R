...
source("functions/8_lin_reg_functions.R")
...
#	traces_norm <- np$load(paste(data_directory, "traces_norm.npy", sep = ""))
...
#    bl_quant <- quant_baseline_metrics(traces, baseL_index)
    bl_norm_quant <- quant_baseline_metrics(traces_norm, baseL_index)
...
#    write.table(bl_norm_quant$noise, paste(data_directory, "baseline_norm_noise.dat", sep = ""), row.names = F, col.names = F)
#    write.table(bl_norm_quant$fluor, paste(data_directory, "baseline_norm_fluor.dat", sep = ""), row.names = F, col.names = F)
}


save_snr_hier_lin_reg <- function(prefix_list, model_name = "lin_reg_snr"){
	...
	dot_r2 <- calc_r2_bayes(dat_dot, mod_fit_dot)


    # MI grat vs SNR

	MI_grat <- mapply(function(MI,NCC) MI[NCC], MI_grat_list, NCC_thresh_list)
	snr <- mapply(function(snr,NCC) snr[NCC], snr_list, NCC_thresh_list)
	...
	grat_r2 <- calc_r2_bayes(dat_grat, mod_fit_grat)



	dat <- list(dot = dat_dot, grat = dat_grat)
	model_output <- list(dot = mod_fit_dot, grat = mod_fit_grat, dot_r2 = dot_r2, grat_r2 = grat_r2)
